...
//    public ManzanaCaptura getManzanaCaptura(String idmanzana){
...
//    }
    ...
    public ArrayList<ManzanaCaptura> getAllManzanaCapturaXEstado(int estado){
        ArrayList<ManzanaCaptura> manzanas = new ArrayList<>();
        Cursor cursor = null;
        try{
            cursor = sqLiteDatabase.rawQuery("SELECT id,iduser,ccdd,ccpp,ccdi,codzona,sufzona,codmzna,sufmzna,mznabelong,estado,frentes,AsGeoJSON(shape) geom FROM manzana_captura where estado="+estado+"",null);
            while(cursor.moveToNext()){
                ManzanaCaptura manzana = new ManzanaCaptura();
                manzana.setId(cursor.getInt(0));
                manzana.setIduser(cursor.getInt(1));
    ...
    public void updateManzanaCapturaXEstado(String codmzna,int estado){
        sqLiteDatabase.execSQL("UPDATE manzana_captura SET estado="+estado+" WHERE codmzna='"+codmzna+"';");
    }

    public void updateManzanaCapturaXMnzabelong(String codmzna,String mznabelong){
        sqLiteDatabase.execSQL("UPDATE manzana_captura SET mznabelong='"+mznabelong+"' WHERE codmzna='"+codmzna+"';");
    }
